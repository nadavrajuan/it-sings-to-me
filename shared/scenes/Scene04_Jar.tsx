...
export default function Scene04_Jar({ progress, aspectRatio, backgroundSlot, screenshotSrc }: SceneProps) {
  ...
  const shotOpacity  = lerp(progress, [0.28, 0.50], [0, 1]);
  const shotY        = lerp(progress, [0.28, 0.50], [30, 0]);

  const titlePx = isVertical ? 72 : 82;
  const bodyPx  = isVertical ? 28 : 34;
  const padH    = isVertical ? 70 : 100;
  const padB    = isVertical ? 130 : 90;
  const shotW   = isVertical ? 700 : 820;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', opacity, background: palette.ink }}>
      ...
      {/* Etymology table screenshot — centred in upper region */}
      {shotOpacity > 0 && (
        <div style={{
          position: 'absolute',
          zIndex: 5,
          left: '50%',
          top: isVertical ? '10%' : '6%',
          transform: `translateX(-50%) translateY(${shotY}px) rotate(-1.5deg)`,
          width: shotW,
          opacity: shotOpacity,
          ...
          <img
            src={screenshotSrc ?? '/assets/screenshots/gpt-etymology.png'}
            alt=""
            onError={(e) => ((e.target as HTMLImageElement).style.display = 'none')}
